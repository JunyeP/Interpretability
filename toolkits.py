...
def validate(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for batch_idx, (images, labels) in enumerate(progress_bar):
            ...
            masked_acc = (masked_preds == labels).sum().item() / labels.size(0) * 100
            unmasked_acc = (unmasked_preds == labels).sum().item() / labels.size(0) * 100
            ...
            val_metrics['masked_acc'] += masked_acc
            val_metrics['unmasked_acc'] += unmasked_acc
    ...
    for key in val_metrics:
        if key != 'validation_time':  # Don't average the time
...
def train_epoch(model, train_loader, criterion, optimizer, device):
    ...
    for key in train_metrics:
        if key != 'training_time':  # Don't average the time
